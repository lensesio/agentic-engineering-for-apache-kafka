...
1. **Gather context** - Review the pre-computed state above
...
The current state has been pre-computed above via inline bash. Review it to understand what's being committed.
...
- Follow the project's existing commit message style from the recent commits above
